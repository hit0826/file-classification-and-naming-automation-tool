...
if "%~1"=="" (
  set /p TARGET=Folder to organize: 
) else (
  set "TARGET=%~1"
)
set "TARGET=%TARGET:"=%"
...
py "%SCRIPT%" organize "!TARGET!" --recursive --rules "%~dp0rules.example.json" --mode "!MODE!" --apply
echo.
echo Done. Choose another job, change folder, or exit.
